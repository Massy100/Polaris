...
def _normalize_row(row: dict) -> dict:
    return {_normalize_key(str(key)): ("" if value is None else str(value).strip()) for key, value in row.items()}
...
def _process_file(category: str, file_obj) -> tuple[BulkUploadBatch, list[dict]]:
...
def bulk_upload(request):
    ...
    if category not in ALLOWED_CATEGORIES:
        return _add_cors_headers(
            JsonResponse(
                {
                    "ok": False,
                    "message": "Categoria invalida. Usa: titulos, meritos u opiniones.",
                },
                status=400,
            )
        )
    ...
    if not files:
        return _add_cors_headers(
            JsonResponse({"ok": False, "message": "No se recibieron archivos."}, status=400)
        )
        ...
        if file_obj.size > MAX_FILE_SIZE_BYTES:
            return _add_cors_headers(
                JsonResponse(
                    {
                        "ok": False,
                        "message": f"Archivo demasiado grande: {filename}. Maximo 10MB.",
                    },
                    status=400,
                )
            )
        ...
        try:
            ...
            return _add_cors_headers(
                JsonResponse(
                    {
                        "ok": False,
                        "message": f"No se pudo procesar {filename}: {str(exc)}",
                    },
                    status=400,
                )
            )
